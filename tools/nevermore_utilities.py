...
class TransportSerial(Transport):
    ...
    class Attribute(Transport.Attribute):
        ...
        async def write(self, blob: bytes):
            ...
            self.serial._read_until(TransportSerial.CMD_ATTR_WRITE)
            handle = self.serial._read_uint(2)
            okay = self.serial._read_uint(1)
        ...
        async def read(self):
            ...
            self.serial._read_until(TransportSerial.CMD_ATTR_READ)
            handle = self.serial._read_uint(2)
            okay = self.serial._read_uint(1)
            data = self.serial._read_payload()
            if handle != self.handle:
                raise Exception(f"replied w/ handle {handle}, expected {self.handle}")
            if not okay:
    ...
    def __init__(self, serial: serial.Serial):
        ...
        for attr in attrs:
            if attr.uuid == UUID_CHAR_GATT:
                ...
                attrs[index] = dataclasses.replace(attrs[index], flags=gatt_flags)

        # partition into services
        for attr in attrs:
            if attr.uuid == UUID_SERVICE_PRIMARY:
                self.services.append(TransportSerial.Service(self, attr, []))
    ...
    def _read_until(self, cmd: int):
        data = self.serial.read_until(bytes([cmd]))
        if not data or data[-1] != cmd:
            raise TransportSerial.TimeoutException(read=True)
