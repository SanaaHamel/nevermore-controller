...
struct SHT4x final : SensorI2C<Cmd, "SHT4x", 0xFF> {
    ...
    bool issue(EnvState const&) override {
        return i2c.touch(Cmd::MeasureHi);
    }
    ...
    pair<EnvState, optional<uint16_t>> readback() override {
        ...
        return {{._temperature = m->temperature, ._humidity = clamp(m->humidity, 0.f, 100.f)}, {}};
    }
...
private:
    ...
    optional<pair<uint16_t, uint16_t>> read6() {
...
unique_ptr<Sensor> sht4x(Pins::BusI2C const& pins) {
    for (auto address : ADDRESSES)
        if (auto p = make_unique<SHT4x>(pins, address); p->setup()) return p;
