...
void stdio_usb_out_chars(const char* buf, int length) {
    if (!xSemaphoreTake(g_stdio_usb_mutex, pdMS_TO_TICKS(PICO_STDIO_DEADLOCK_TIMEOUT_MS))) return;
    SCOPE_GUARD {
...
int stdio_usb_in_chars(char* buf, int length) {
    ...
    if (!xSemaphoreTake(g_stdio_usb_mutex, pdMS_TO_TICKS(PICO_STDIO_DEADLOCK_TIMEOUT_MS)))
