...
void SensorPeriodic::update(async_context_t& context) {
    auto const bgn = std::chrono::steady_clock::now();
    read();
...
void SensorPeriodic::update_enqueue(
        async_context_t& context, std::chrono::steady_clock::duration update_duration) {
    // printf("sensor update enqueue - %s - duration %d us\n", name(), int(update_duration / 1us));
    auto const delay_ms = std::max<int64_t>((update_period() - update_duration) / 1ms, 0);
...
void SensorDelayedResponse::update(async_context_t& context) {
    update_bgn = std::chrono::steady_clock::now();
    ...
    if (!issue()) {
        ...
        update_enqueue(context, std::chrono::steady_clock::now() - update_bgn);
        return;
    }
...
void SensorDelayedResponse::read_dispatcher(async_context_t* context, async_work_on_timeout* work) {
    ...
    self.read();
    self.update_enqueue(*context, std::chrono::steady_clock::now() - self.update_bgn);
}
