...
int I2C_PIO::write(uint8_t addr, uint8_t const* src, size_t len, bool stop = true) {
    assert(stop && "!stop not-implemented");
    return pio_i2c_write_timeout_us(pio, sm, addr, src, len, I2C_TIMEOUT_US);
}

int I2C_PIO::read(uint8_t addr, uint8_t* dst, size_t len, bool stop = true) {
    assert(stop && "!stop not-implemented");
